...
import PersonEditInner from '@/components/main/people/modal/detail/edit/EditInner'
import {
  AlertDialog,
  AlertDialogCancel,
  AlertDialogContent,
...
import { useGetPersonQuery } from '@/lib/gql/graphql'
...
export default function CompanyDetailEditModal(props: Props) {
  ...
  const person = data?.getPerson as Person
  ...
  return (
    <AlertDialog open={props.isOpen} onOpenChange={props.setIsOpen}>
      <AlertDialogContent onCloseAutoFocus={() => router.back()}>
        ...
        <AlertDialogTitle>Edit Person</AlertDialogTitle>
        <AlertDialogDescription>
          {loading && <p>Loading...</p>}
          {error && <p>Error:{error.message}</p>}
          {person && (
            <PersonEditInner person={person} setIsOpen={props.setIsOpen} />
          )}
        </AlertDialogDescription>
      </AlertDialogContent>
    </AlertDialog>
